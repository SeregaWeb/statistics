 ...
 * Create new trailer
 */
export const createTrailer = (urlAjax) => {
    const form = document.querySelector('.js-create-trailer');
    if (!form) return;
    ...
    form.addEventListener('submit', (e: Event) => {
        ...
        const allInputs = target.querySelectorAll('input[type="number"], input[type="text"], input[type="checkbox"], select');
...
/**
 * Upload trailer file helper
 */
export const uploadFileTrailer = (ajaxUrl) => {
    const forms = document.querySelectorAll('.js-upload-trailer-helper');
    ...
    forms.forEach((form) => {
        ...
        form.addEventListener('submit', (e) => {
            ...
            const action = 'upload_trailer_helper';
    ...
    deleteForms &&
        deleteForms.forEach((item) => {
            item.addEventListener('submit', (event) => {
                ...
                // @ts-ignore
                const formData = new FormData(target);
                const action = 'remove_one_trailer';
...
const handleCreateButton = (urlAjax) => {
    const createBtn = document.querySelector('.js-submit-create-trailer');
    if (!createBtn) return;

    createBtn.addEventListener('click', (e) => {
        e.preventDefault();
        const form = document.querySelector('.js-create-trailer') as HTMLFormElement;
        if (!form) return;
 ...
 * @param fieldName - Name of the field (e.g., 'license_plate_file', 'trailer_registration', 'lease_agreement')
...
const updateFileUploadUI = (fieldName: string, isUploaded: boolean) => {
    ...
    if (isUploaded) {
        ...
        if (uploadBtn) {
            ...
            // For License Plate (special layout)
            if (fieldName === 'license_plate_file') {
                ...
                // For other fields (simple-file-upload component)
 ...
 */
const deleteTrailer = (ajaxUrl: string) => {
    const deleteButtons = document.querySelectorAll('.js-delete-trailer, .js-delete-trailer-single');
    ...
    deleteButtons.forEach((button) => {
        button.addEventListener('click', (e) => {
            ...
            if (!trailerId) {
                printMessage('Trailer ID not found', 'danger', 5000);
                return;
            }
            ...
            fetch(ajaxUrl, options)
                ...
                .then((requestStatus) => {
                    if (requestStatus.success) {
                        printMessage(requestStatus.data.message || 'Trailer deleted successfully', 'success', 5000);
                        
                        // Check if we're on list page or single page
                        const isListPage = btn.classList.contains('js-delete-trailer');
                        const isSinglePage = btn.classList.contains('js-delete-trailer-single');
                        ...
                        if (isListPage) {
                            ...
                            // Redirect to trailers list page after deletion
                            const trailersUrlInput = document.querySelector('#trailers-list-url') as HTMLInputElement;
                            const trailersListUrl = trailersUrlInput?.value;
                            ...
                            if (trailersListUrl) {
                                ...
                                const trailersLink = document.querySelector('a[href*="trailers"]:not([href*="trailer-add"])');
                                if (trailersLink) {
                                    setTimeout(() => {
                                        window.location.href = (trailersLink as HTMLAnchorElement).href;
                                    }, 1500);
                                } else {
                        ...
                        const errorMessage = requestStatus.data?.message || 'Error deleting trailer';
                        printMessage(errorMessage, 'danger', 8000);
                    }
                })
                .catch((error) => {
...
/**
 * Initialize all trailer actions
 */
export const trailersActions = (urlAjax) => {
