...
if ( ! empty( $hold_drivers ) ) :
	...
	/**
	 * Determine button color based on rating value
	 *
	 * @param float|string $value Rating value
	 * @return string Button color class
	 */
	...
	<div class="hold-drivers-section mb-4">
		<div class="d-flex justify-content-between align-items-center mb-3">
			<h6 class="mb-0 d-flex align-items-center">
				On hold now
				<span class="badge bg-primary ms-2" style="font-size: 0.75rem;"><?php echo esc_html( count( $hold_drivers ) ); ?></span>
			</h6>
			<button class="btn btn-sm btn-outline-secondary js-toggle-hold-section" type="button">
			...
			<table class="table mb-5 w-100">
				...
				<tbody>
					<?php foreach ( $hold_drivers as $driver ) :
						...
						$show_phone_field  = get_field_value( $meta, 'show_phone' ) ?? 'driver_phone';
						$driver_phone      = get_field_value( $meta, $show_phone_field );
						...
						$hold_info         = $driver['hold_info'] ?? null;
						
						// Check if current user can see phone number
						$can_see_phone = ( $hold_info && $current_user_id === (int) ( $hold_info['dispatcher_id'] ?? 0 ) );
						
						// Get driver statistics for rating
						$driver_statistics = $drivers->get_driver_statistics( $driver_id );
						$avg_rating        = floatval( $driver_statistics['rating']['avg_rating'] ?? 0 );
						$notice_count      = absint( $driver_statistics['notice']['count'] ?? 0 );
						
						// Format location string
						$location_display = '';
						if ( ! empty( $current_city ) && ! empty( $current_location ) ) {
							$location_display = esc_html( $current_city . ', ' . $current_location );
						} else {
							$location_display = '<span class="text-muted">Need to set this field</span>';
						}
						
						// Format date
						$date_available = '';
						if ( ! empty( $driver['date_available'] ) ) {
							$timestamp = strtotime( $driver['date_available'] );
							if ( $timestamp !== false ) {
								$date_available = esc_html( date( 'm/d/Y g:i a', $timestamp ) );
							}
						}
						
						// Format vehicle info
						$vehicle_display = '';
						$vehicle_parts   = array_filter( [ $vehicle_model, $vehicle_make, $vehicle_year ] );
						if ( ! empty( $vehicle_parts ) ) {
							$vehicle_display = esc_html( implode( ' ', $vehicle_parts ) );
						}
						
						// Format payload
						$payload_display = '';
						if ( ! empty( $payload ) ) {
							$payload_display = esc_html( $payload ) . ' - lbs';
						}
						?>
						<tr>
							<td style="width: 100px;" class="on_hold driver-status">On hold</td>

							<td class="table-column js-location-update" style="font-size: 12px; width: 200px;">
								<?php echo $location_display; ?>
								<?php if ( ! empty( $date_available ) ) : ?>
									<?php echo ' <br>' . $date_available; ?>
								<?php endif; ?>
							</td>
							...
							<td>
								<div class="d-flex flex-column">
									<div>
										<?php echo '(' . esc_html( $driver_id ) . ') ' . esc_html( $driver_name ); ?>
										<?php echo $icons->get_flags( $languages ); ?>
									</div>
									<span class="text-small">
										<?php if ( $can_see_phone && ! empty( $driver_phone ) ) : ?>
							...
							<td>
								<div class="d-flex flex-column">
									<?php if ( ! empty( $vehicle_type ) && isset( $driverHelper->vehicle[ $vehicle_type ] ) ) : ?>
										<?php echo esc_html( $driverHelper->vehicle[ $vehicle_type ] ); ?>
									<?php endif; ?>
									<span class="text-small">
										<?php echo $vehicle_display; ?>
									</span>
								</div>
							</td>
							...
							<td>
								<div class="table-tags d-flex flex-wrap">
									<?php if ( $hold_info ) : ?>
										<div style="font-size: 12px; color: #666; line-height: 1.2; min-width: 186px;">
											On hold by<br>
							...
							<td style="width: 252px;">
								<?php if ( $hold_info && ! empty( $hold_info['minutes_left'] ) ) : ?>
									Will be available in<br><?php echo esc_html( $hold_info['minutes_left'] ); ?> min
								<?php else : ?>
							...
							<td style="width: 86px;">
								<?php if ( $can_see_phone ) : ?>
									<a target="_blank"
									   href="<?php echo esc_url( $add_new_load . '?driver=' . $driver_id . '&tab=pills-driver-stats-tab' ); ?>"
									   class="btn <?php echo esc_attr( $get_button_color( $avg_rating ) ); ?> btn-sm d-flex align-items-center justify-content-between gap-1">
										<?php echo $avg_rating > 0 ? esc_html( $avg_rating ) : '-'; ?>
								...
								<?php else : ?>
									<span class="btn <?php echo esc_attr( $get_button_color( $avg_rating ) ); ?> btn-sm d-flex align-items-center justify-content-between gap-1 disabled"
								...
								<?php if ( $can_see_phone ) : ?>
									<a target="_blank"
									   ...
									   class="btn btn-primary btn-sm d-flex align-items-center justify-content-between gap-1">
										<?php echo $notice_count > 0 ? esc_html( $notice_count ) : '-'; ?>
							...
							<td style="width: 92px;">
                                ...
                                <button class="btn btn-sm d-flex align-items-center justify-content-center js-hold-driver"
											...
											data-id="<?php echo esc_attr( $driver_id ); ?>"
											data-dispatcher="<?php echo esc_attr( $current_user_id ); ?>"
											data-hold="null">
									...
									<button class="btn btn-sm d-flex align-items-center justify-content-center js-hold-driver"
											...
											data-id="<?php echo esc_attr( $driver_id ); ?>"
											data-dispatcher="<?php echo esc_attr( $current_user_id ); ?>"
											data-hold="<?php echo esc_attr( $hold_info ? ( $hold_info['dispatcher_id'] ?? 'null' ) : 'null' ); ?>">
										<svg style="width: 16px; height: 16px; pointer-events: none; enable-background:new 0 0 511.992 511.992;"
											 xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
											 version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 511.992 511.992"
