...
class TMSDrivers extends TMSDriversHelper {
	...
	public function add_driver() {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			$data = array(
				...
				'mc_enabled'                 => isset( $_POST[ 'mc_enabled' ] ) ? 'on' : '',
				'mc'                         => isset( $_POST[ 'mc' ] ) ? sanitize_text_field( $_POST[ 'mc' ] ) : '',
				'dot_enabled'                => isset( $_POST[ 'dot_enabled' ] ) ? 'on' : '',
	...
	public function get_driver_loads( $driver_id, $for_rating = false ) {
		...
		// Build WHERE conditions - check attached_driver, attached_second_driver and attached_third_driver
		$where_conditions = array("(rm.meta_value = %s OR rm2.meta_value = %s OR rm3.meta_value = %s)", "r.date_created >= %s");
		$where_values = array((string)$driver_id, (string)$driver_id, (string)$driver_id, '2025-10-01 00:00:00');
		...
		$regular_query = "
			SELECT DISTINCT r.id, load_status_meta.meta_value as load_status, ref_meta.meta_value as load_number, r.date_created, 'regular' as load_type
			...
			LEFT JOIN $reports_meta_table load_status_meta ON r.id = load_status_meta.post_id AND load_status_meta.meta_key = 'load_status'";
		
		// Add rating-related joins if requested
		if ( $for_rating ) {
			// Add LEFT JOIN for tbd (load_status already joined above)
			$regular_query .= "
		...
		if ( $flt_meta_table_exists && $has_flt_access ) {
			// Build WHERE conditions for FLT query (use rfm instead of rm) - check attached_driver, attached_second_driver and attached_third_driver
			$flt_where_conditions = array("(rfm.meta_value = %s OR rfm2.meta_value = %s OR rfm3.meta_value = %s)", "rf.date_created >= %s");
			$flt_where_values = array((string)$driver_id, (string)$driver_id, (string)$driver_id, '2025-10-01 00:00:00');
			...
			$flt_query = "
				SELECT DISTINCT rf.id, load_status_meta.meta_value as load_status, ref_meta.meta_value as load_number, rf.date_created, 'flt' as load_type
				...
				LEFT JOIN $reports_flt_meta_table load_status_meta ON rf.id = load_status_meta.post_id AND load_status_meta.meta_key = 'load_status'";
	 ...
	 * @param array $exclude_dispatchers Array of dispatcher IDs to exclude from statistics
	 * @return array Array of dispatcher statistics
	 */
	public function get_dispatcher_rating_statistics( $year = null, $month = null, $is_flt = null, $exclude_dispatchers = array() ) {
		global $wpdb;
		...
		// Exclude specified dispatchers if provided
		if ( ! empty( $exclude_dispatchers ) && is_array( $exclude_dispatchers ) ) {
			$query_args['exclude'] = array_map( 'intval', $exclude_dispatchers );
		}
		
		$all_dispatchers = get_users( $query_args );
	...
	public function ajax_add_driver_rating() {
		...
		// If load_number is provided, try to detect load status by reference_number
		// If status is 'tonu', prepend (TONU) to comments
		if ( ! empty( $load_number ) ) {
			global $wpdb;
			
			// Detect current project (same logic as in get_driver_financial_statistics)
			$current_user_id  = get_current_user_id();
			...
			// Try to find status in regular reports by reference_number
			// Use TRIM to handle potential whitespace issues
			$regular_status_query = $wpdb->prepare(
				"
				SELECT ls.meta_value AS load_status
				FROM {$reports_table} r
				INNER JOIN {$reports_meta_table} ref
					ON r.id = ref.post_id
					AND ref.meta_key = 'reference_number'
					AND TRIM(ref.meta_value) = %s
				LEFT JOIN {$reports_meta_table} ls
					ON r.id = ls.post_id
					AND ls.meta_key = 'load_status'
				LIMIT 1
				",
				trim( $load_number )
			);
			
			$load_status = $wpdb->get_var( $regular_status_query );
			
			// Debug: log the query result
			error_log( "TMS Rating: Regular query for load_number '{$load_number}' returned status: " . ( $load_status ? $load_status : 'NULL' ) );
			
			// If not found in regular reports, try FLT reports
			if ( empty( $load_status ) ) {
				$flt_status_query = $wpdb->prepare(
					"
					SELECT ls.meta_value AS load_status
					FROM {$reports_flt_table} rf
					INNER JOIN {$reports_flt_meta_table} ref
						ON rf.id = ref.post_id
						AND ref.meta_key = 'reference_number'
						AND TRIM(ref.meta_value) = %s
					LEFT JOIN {$reports_flt_meta_table} ls
						ON rf.id = ls.post_id
						AND ls.meta_key = 'load_status'
					LIMIT 1
					",
				...
				$load_status = $wpdb->get_var( $flt_status_query );
				
				// Debug: log the query result
				error_log( "TMS Rating: FLT query for load_number '{$load_number}' returned status: " . ( $load_status ? $load_status : 'NULL' ) );
			}
			
			// If status is TONU, prepend marker to comments
			if ( ! empty( $load_status ) && strtolower( trim( $load_status ) ) === 'tonu' ) {
				// Only prepend if not already present
				if ( strpos( $comments, '(TONU)' ) === false ) {
					$comments = '(TONU) ' . $comments;
				}
			}
			
			// Debug logging
			error_log( "TMS Rating: load_number={$load_number}, load_status={$load_status}, comments_before={$comments}" );
		}
	...
	public function get_driver_financial_statistics( $driver_id ) {
		...
		if ( $regular_table_exists && $regular_meta_exists ) {
			...
			$stats_query = "
				SELECT 
					...
					COALESCE(SUM(
						CASE 
							...
							WHEN rm_third_driver.meta_value = %d THEN CAST(rm_third_driver_rate.meta_value AS DECIMAL(10,2))
							ELSE 0
						END
				...
				LEFT JOIN {$regular_meta_table} rm_third_driver ON r.ID = rm_third_driver.post_id AND rm_third_driver.meta_key = 'attached_third_driver'
				...
				LEFT JOIN {$regular_meta_table} rm_third_driver_rate ON r.ID = rm_third_driver_rate.post_id AND rm_third_driver_rate.meta_key = 'third_driver_rate'
		...
		if ( $access_flt || in_array( 'administrator', $user_roles ) ) {
			...
			if ( $flt_table_exists && $flt_meta_exists ) {
				...
				$flt_debug_load_ids_query = "
					...
					LEFT JOIN {$flt_meta_table} rfm_third_driver ON rf.ID = rfm_third_driver.post_id AND rfm_third_driver.meta_key = 'attached_third_driver'
					...
					AND (%d IN (rfm_driver.meta_value, rfm_second_driver.meta_value, rfm_third_driver.meta_value))
					ORDER BY rf.ID
				";
				...
				$flt_stats_query = "
					SELECT 
						...
						COALESCE(SUM(
							CASE 
								...
								WHEN rfm_third_driver.meta_value = %d THEN CAST(rfm_third_driver_rate.meta_value AS DECIMAL(10,2))
								ELSE 0
							END
						), 0) as total_driver_earnings
					FROM {$flt_table} rf
					...
					LEFT JOIN {$flt_meta_table} rfm_third_driver_rate ON rf.ID = rfm_third_driver_rate.post_id AND rfm_third_driver_rate.meta_key = 'third_driver_rate'
					LEFT JOIN {$flt_meta_table} rfm_profit ON rf.ID = rfm_profit.post_id AND rfm_profit.meta_key = 'profit'
					WHERE rfm_status.meta_value = 'delivered'
					AND (%d IN (rfm_driver.meta_value, rfm_second_driver.meta_value, rfm_third_driver.meta_value))
				";
				
				$flt_results = $wpdb->get_row( $wpdb->prepare( $flt_stats_query, $driver_id, $driver_id, $driver_id, $driver_id ) );
	...
	public function get_top_drivers($limit = 25) {
		...
		// Step 3.5: Get third driver load counts (only for drivers that exist in drivers table)
		$third_driver_loads_query = "
			SELECT 
				rm_third_driver.meta_value as driver_id,
				COUNT(DISTINCT r.ID) as third_loads
			FROM {$reports_table} r
			LEFT JOIN {$reports_meta_table} rm_third_driver ON r.ID = rm_third_driver.post_id AND rm_third_driver.meta_key = 'attached_third_driver'
			LEFT JOIN {$reports_meta_table} rm_status ON r.ID = rm_status.post_id AND rm_status.meta_key = 'load_status'
			INNER JOIN {$drivers_table} d ON rm_third_driver.meta_value = d.id
			WHERE rm_status.meta_value = 'delivered'
			AND rm_third_driver.meta_value IN ($placeholders)
			GROUP BY rm_third_driver.meta_value
		";
		
		$third_driver_loads = $wpdb->get_results($wpdb->prepare($third_driver_loads_query, $driver_ids));
		...
		// Step 5.5: Get third driver financial data (only for drivers that exist in drivers table)
		$third_driver_financial_query = "
			SELECT 
				rm_third_driver.meta_value as driver_id,
				COALESCE(SUM(CAST(rm_profit.meta_value AS DECIMAL(10,2))), 0) as third_profit,
				COALESCE(SUM(CAST(rm_third_rate.meta_value AS DECIMAL(10,2))), 0) as third_earnings
			FROM {$reports_table} r
			LEFT JOIN {$reports_meta_table} rm_third_driver ON r.ID = rm_third_driver.post_id AND rm_third_driver.meta_key = 'attached_third_driver'
			LEFT JOIN {$reports_meta_table} rm_status ON r.ID = rm_status.post_id AND rm_status.meta_key = 'load_status'
			LEFT JOIN {$reports_meta_table} rm_profit ON r.ID = rm_profit.post_id AND rm_profit.meta_key = 'profit'
			LEFT JOIN {$reports_meta_table} rm_third_rate ON r.ID = rm_third_rate.post_id AND rm_third_rate.meta_key = 'third_driver_rate'
			INNER JOIN {$drivers_table} d ON rm_third_driver.meta_value = d.id
			WHERE rm_status.meta_value = 'delivered'
			AND rm_third_driver.meta_value IN ($placeholders)
			GROUP BY rm_third_driver.meta_value
		...
		$third_driver_financial = $wpdb->get_results($wpdb->prepare($third_driver_financial_query, $driver_ids));
		...
		if ($use_flt) {
			...
			// FLT third driver loads (only for drivers that exist in drivers table)
			$flt_third_loads_query = "
				SELECT 
					rfm_third_driver.meta_value as driver_id,
					COUNT(DISTINCT rf.ID) as third_loads
				FROM {$reports_flt_table} rf
				LEFT JOIN {$reports_flt_meta_table} rfm_third_driver ON rf.ID = rfm_third_driver.post_id AND rfm_third_driver.meta_key = 'attached_third_driver'
				LEFT JOIN {$reports_flt_meta_table} rfm_status ON rf.ID = rfm_status.post_id AND rfm_status.meta_key = 'load_status'
				INNER JOIN {$drivers_table} d ON rfm_third_driver.meta_value = d.id
				WHERE rfm_status.meta_value = 'delivered'
				AND rfm_third_driver.meta_value IN ($placeholders)
				GROUP BY rfm_third_driver.meta_value
			";
			$flt_third_loads = $wpdb->get_results($wpdb->prepare($flt_third_loads_query, $driver_ids));
			...
			// FLT third driver financial (only for drivers that exist in drivers table)
			$flt_third_financial_query = "
				SELECT 
					rfm_third_driver.meta_value as driver_id,
					COALESCE(SUM(CAST(rfm_profit.meta_value AS DECIMAL(10,2))), 0) as third_profit,
					COALESCE(SUM(CAST(rfm_third_rate.meta_value AS DECIMAL(10,2))), 0) as third_earnings
				FROM {$reports_flt_table} rf
				LEFT JOIN {$reports_flt_meta_table} rfm_third_driver ON rf.ID = rfm_third_driver.post_id AND rfm_third_driver.meta_key = 'attached_third_driver'
				LEFT JOIN {$reports_flt_meta_table} rfm_status ON rf.ID = rfm_status.post_id AND rfm_status.meta_key = 'load_status'
				LEFT JOIN {$reports_flt_meta_table} rfm_profit ON rf.ID = rfm_profit.post_id AND rfm_profit.meta_key = 'profit'
				LEFT JOIN {$reports_flt_meta_table} rfm_third_rate ON rf.ID = rfm_third_rate.post_id AND rfm_third_rate.meta_key = 'third_driver_rate'
				INNER JOIN {$drivers_table} d ON rfm_third_driver.meta_value = d.id
				WHERE rfm_status.meta_value = 'delivered'
				AND rfm_third_driver.meta_value IN ($placeholders)
				GROUP BY rfm_third_driver.meta_value
			";
			$flt_third_financial = $wpdb->get_results($wpdb->prepare($flt_third_financial_query, $driver_ids));
		}
		...
		$third_loads_lookup = [];
		foreach ($third_driver_loads as $row) {
			$third_loads_lookup[intval($row->driver_id)] = intval($row->third_loads);
		}
		...
		$third_financial_lookup = [];
		foreach ($third_driver_financial as $row) {
			$third_financial_lookup[intval($row->driver_id)] = [
				'profit' => floatval($row->third_profit),
				'earnings' => floatval($row->third_earnings)
			];
		}
		...
		foreach ($flt_third_loads as $row) {
			$driver_id = intval($row->driver_id);
			$third_loads_lookup[$driver_id] = ($third_loads_lookup[$driver_id] ?? 0) + intval($row->third_loads);
		}
		...
		foreach ($flt_third_financial as $row) {
			$driver_id = intval($row->driver_id);
			$third_financial_lookup[$driver_id] = [
				'profit' => ($third_financial_lookup[$driver_id]['profit'] ?? 0) + floatval($row->third_profit),
				'earnings' => ($third_financial_lookup[$driver_id]['earnings'] ?? 0) + floatval($row->third_earnings)
			];
		}
		...
		foreach ($driver_ids as $driver_id) {
			...
			$third_loads = $third_loads_lookup[$driver_id] ?? 0;
			$total_loads = $first_loads + $second_loads + $third_loads;
			...
			if ($total_loads > 0) {
				...
				$third_financial = $third_financial_lookup[$driver_id] ?? ['profit' => 0, 'earnings' => 0];
				...
				$driver_stats[] = [
					...
					'third_driver_loads' => $third_loads,
					'total_profit' => $first_financial['profit'] + $second_financial['profit'] + $third_financial['profit'],
					'total_earnings' => $first_financial['earnings'] + $second_financial['earnings'] + $third_financial['earnings']
				];
			}
		}
	...
	/**
	 * Get top drivers by ratings (quantity and quality)
	 * Criteria: minimum 5 ratings, sorted by rating count (desc), then by average rating (desc)
	 * 
	 * @param int $limit Maximum number of drivers to return
	 * @param int $min_ratings Minimum number of ratings required (default: 5)
	 * @return array Array of driver statistics
	 */
	public function get_top_drivers_by_ratings($limit = 25, $min_ratings = 5) {
		global $wpdb;
		...
		// Get drivers with minimum ratings requirement, sorted by rating then count
		// This ensures that drivers with higher ratings are prioritized, even if they have slightly fewer ratings
		$ratings_query = "
			SELECT 
				r.driver_id,
				COUNT(r.id) as rating_count,
				ROUND(AVG(r.reit), 2) as avg_rating,
				main.status_post
			FROM {$drivers_rating_table} r
			INNER JOIN {$drivers_table} main ON r.driver_id = main.id
		...
		$rated_drivers = $wpdb->get_results($wpdb->prepare($ratings_query, $min_ratings, $limit));
		
		if (empty($rated_drivers)) {
			return [];
		}
		...
		// Create rating lookup arrays
		$rating_lookup = [];
		$rating_count_lookup = [];
		foreach ($rated_drivers as $driver) {
			$driver_id = intval($driver->driver_id);
			$rating_lookup[$driver_id] = floatval($driver->avg_rating);
			$rating_count_lookup[$driver_id] = intval($driver->rating_count);
		}
		
		// Get driver names and recruiter IDs
		$drivers_info_query = "
			SELECT 
				main.id as driver_id,
				name_meta.meta_value as driver_name,
				main.user_id_added as recruiter_id
			FROM {$drivers_table} main
			LEFT JOIN {$drivers_meta_table} name_meta ON main.id = name_meta.post_id AND name_meta.meta_key = 'driver_name'
			WHERE main.id IN ($placeholders)
			AND main.status_post = 'publish'
		";
		
		$drivers_info = $wpdb->get_results($wpdb->prepare($drivers_info_query, $driver_ids));
		...
		foreach ($drivers_info as $driver) {
			$driver_id = intval($driver->driver_id);
			$name_lookup[$driver_id] = $driver->driver_name ?: 'Unknown';
			$recruiter_lookup[$driver_id] = intval($driver->recruiter_id);
		}
		
		// Build result array maintaining the order from ratings query
		$driver_stats = [];
		foreach ($rated_drivers as $driver) {
			$driver_id = intval($driver->driver_id);
			...
			$driver_stats[] = [
				...
				'recruiter_id' => $recruiter_lookup[$driver_id] ?? 0,
				'rating' => $rating_lookup[$driver_id] ?? 0.0,
				'rating_count' => $rating_count_lookup[$driver_id] ?? 0,
				'delivered_loads' => 0, // Not needed for this view, but keeping structure consistent
				'first_driver_loads' => 0,
				'second_driver_loads' => 0,
				'third_driver_loads' => 0,
				'total_profit' => 0, // Not needed for this view
				'total_earnings' => 0 // Not needed for this view
			];
		}
	...
	/**
	 * AJAX handler for updating driver auto block exclude setting
	 */
	public function ajax_update_driver_auto_block_exclude() {
		if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
			return;
		}

		$driver_id = intval( $_POST['driver_id'] ?? 0 );
		$exclude = isset( $_POST['exclude_from_auto_block'] ) && $_POST['exclude_from_auto_block'] === '1' ? '1' : '0';
		...
		$existing = $wpdb->get_var( $wpdb->prepare( "
			...
			WHERE post_id = %d AND meta_key = 'exclude_from_auto_block'
		", $driver_id ) );
		...
		if ( $existing ) {
			...
			$result = $wpdb->update(
				$table_meta,
				array( 'meta_value' => $exclude ),
				array(
					'post_id' => $driver_id,
					'meta_key' => 'exclude_from_auto_block'
				),
				array( '%s' ),
				array( '%d', '%s' )
			);
